...
**Checkpoint actual (2026-09-24, rama `codex/mvc-migracion`):** `b5a1d63` amplió `IUserReader` para búsquedas de cuentas y `IdentityService` delega esas lecturas; `6d9a6c5`, `3e1dea6` y `30106c9` migraron login methods y login link; `bde0ed4` y `06d4417` migraron las lecturas de roles/permisos; `a8d0c9d`, `8654157` y `b8ccbd4` migraron las tres lecturas de usuarios. `c8659d4` preparó roles con `IRoleRepository` transaccional y `b8bb5d9` cambió sus tres rutas de escritura; `fcd5934` preparó el pedido de código y `da5c801` cambió su ruta HTTP. `ac3984f` preparó la verificación y `9ff4d80` cambió su ruta, corrigiendo además una fuga de código/número en logs automáticos de MVC detectada por las pruebas. `a140dc7` preparó alta/edición de usuarios sin cambiar sus rutas. **15 de las 41 combinaciones** ya usan controllers MVC; las otras 26 siguen atendidas por la implementación anterior. La solución compiló con **0 advertencias y 0 errores** y la suite backend completa pasó **1440/1440, sin omitidos** sobre este estado. No se tocó `main`, el frontend ni la instancia de Aspire de Visual Studio.
