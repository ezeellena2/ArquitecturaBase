...
**Checkpoint actual (2026-09-24, rama `codex/mvc-migracion`):** `b5a1d63` amplió `IUserReader` para búsquedas de cuentas y `IdentityService` delega esas lecturas; `6d9a6c5`, `3e1dea6` y `30106c9` migraron los tres contratos de cuenta de login methods/login link; `bde0ed4` y `06d4417` migraron las lecturas de roles/permisos; `a8d0c9d`, `8654157` y `b8ccbd4` migraron las tres lecturas de usuarios y conectaron `UserService` directamente a `IUserReader`. `fcd5934` preparó `RequestLoginCodeAsync` sin cambiar todavía la ruta; `c8659d4` preparó las escrituras de roles con `IRoleRepository` transaccional sin cambiar todavía sus rutas. **10 de las 41 combinaciones** ya usan controllers MVC; las otras 31 siguen atendidas por la implementación anterior. Tras integrar `c8659d4`, la solución compiló con **0 advertencias y 0 errores** y la suite backend completa pasó **1423/1423, sin omitidos**. No se tocó `main`, el frontend ni la instancia de Aspire de Visual Studio.
...
|---|---|---|---|
| 1 | `POST /account/login-code` → `AccountController`/`IAccountService` | Servicio listo en `fcd5934`; cambio HTTP en worktree separado | `LoginCodeEndpointsTests`, rate limits, contrato Auth, inventario 41 y arquitectura. |
| 2 | `POST/PUT/DELETE /api/roles` → `RolesController`/`IRoleService` | Servicio y repositorio listos en `c8659d4`; cambio HTTP en worktree separado | `RoleCrudEndpointsTests`, rollback de claims, políticas `roles.manage`, OpenAPI e inventario 41. |
| 3 | Escrituras de usuarios → `UsersController`/`IUserService` | `POST /api/users` y `PUT /api/users/{id:guid}` en preparación, sin cambio HTTP | Creación/edición, Identity, permisos, invitaciones, guardado y rollback donde aplique. |
| 4 | Resto de cuenta, usuarios y perfil | Pendiente, por familias independientes | Tests Auth/Users/Me, contratos de frontend, errores que deben guardar y rutas condicionales. |
| 5 | `/connect`, WhatsApp y retiro del pipeline viejo | Pendiente de servicios y pruebas de protocolo/feature | OpenIddict, webhook crudo/firma, workers, suite completa y revisión manual con el front. |
