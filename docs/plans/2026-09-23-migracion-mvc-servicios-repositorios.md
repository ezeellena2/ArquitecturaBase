...
| 6 | Cinco rutas de usuarios restantes: invitación, activar, desactivar, borrar y desvincular WhatsApp | Pendientes, separar escrituras y pruebas por caso de uso | Último Admin, invitaciones, revocación, locks, soft delete, UoW/rollback y contratos HTTP. |
| 7 | Cinco rutas de perfil restantes: código/vinculación/desvinculación WhatsApp y código/confirmación de correo | Pendientes, primero los errores que deben guardar | `IPersistChangesOnFailure`, sesión, locks, teléfono/correo, 400/415 y rutas condicionales. |
| 8 | Dos rutas externas de Google, siete de `/connect` y dos del webhook | Pendientes, por protocolo/área | Callback/returnUrl, passthrough OpenIddict, firma y cuerpo crudo Meta, workers, inventario 41. |
| 9 | Retiro de `Features`, handlers, `IEndpoint` y decoradores restantes | Solo cuando las 21 rutas y workers estén migrados | Build, suite backend/frontend, base vacía, flujos manuales de ingreso/OIDC/WhatsApp y revisión final. |
