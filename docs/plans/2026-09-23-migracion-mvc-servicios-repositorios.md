...
**Baseline automatizado en el worktree integrador:** `dotnet build ArquitecturaBase.slnx --nologo` pasó con 0 advertencias y 0 errores. La primera corrida de `dotnet test` tuvo 1267/1269: dos casos de `WhatsAppRegistrationTests` recibieron `ObjectDisposedException` del arnés al forzar un fallo de arranque por plantillas vacías. Se ajustó solo esa prueba para invocar la validación de inicio registrada por `AddWhatsApp` y comprobar la clave inválida en `OptionsValidationException.Failures` (commit `ffba42f`). El rerun focalizado pasó 18/18 y la suite completa pasó **1269/1269**, sin cambios en producción. El frontend se verificó más tarde en un worktree aislado; el resultado figura en el checkpoint de cierre.
...
**Checkpoint histórico de la Tarea 1:** se movieron los siete contratos de repositorio y todos los contratos retenidos de `Application/Abstractions`; en ese momento solo quedaban `Messaging` y `Behaviors`. `AddControllers` y `MapControllers` convivían con las rutas aún no migradas, sin duplicarlas. El adaptador MVC de `Result` conservó 200/204 y `ProblemDetails` en tres pruebas HTTP; las pruebas de arquitectura fijaron los límites nuevos. La solución compiló con 0 advertencias y 0 errores, y la suite backend completa pasó **1356/1356** antes del primer cambio de ruta.
...
**Checkpoint histórico de persistencia y cuenta:** `238658c` encapsuló los enlaces pendientes (incluidos los vencidos) para revocar sesiones; `037a9b9` separó la lectura fresca de roles del usuario y los permisos cacheados por rol; `5bd6a4f` trasladó filtros, listado y conteos de usuarios a `UserReader`; `06530fd` trasladó nombres, listado, detalle y comprobación de nombres de roles a `RoleReader`. En paralelo, `6d9a6c5` migró `GET /account/login-methods` a `LoginMethodsController`/`IAccountService`, retirando su handler anterior. La solución compiló con **0 advertencias y 0 errores**, el inventario seguía en **41** verbo/ruta y la suite completa pasó **1392/1392**. En ese corte todavía faltaban otras búsquedas y escrituras de Identity; se integraron después.

**Checkpoint histórico de 23/41 (2026-09-24, rama `codex/mvc-migracion`):** `b5a1d63` amplió `IUserReader` y `IdentityService` delega esas lecturas; `6d9a6c5`, `3e1dea6` y `30106c9` migraron login methods y login link; `bde0ed4` y `06d4417` migraron roles/permisos de lectura; `a8d0c9d`, `8654157` y `b8ccbd4` migraron las tres lecturas de usuarios. `c8659d4` preparó roles con `IRoleRepository` transaccional y `b8bb5d9` cambió sus escrituras; `fcd5934`/`da5c801` migraron el pedido de código; `ac3984f`/`9ff4d80` migraron la verificación y corrigieron una fuga de código/número en logs automáticos de MVC. `a140dc7` preparó alta/edición de usuarios, `69e17ab` extrajo `IUserRepository` real con dos pruebas de rollback y `05ff70c` cambió las dos rutas. `26174f6`/`45817d8`/`c0f8d97` migraron GET/PUT de perfil; `9d5c6a1`/`bb53126` migraron el pedido WhatsApp con ausencia real de la ruta si la función está apagada. `5a84f9b` preparó el servicio Google y `29f23af` los servicios de correo del perfil; `f93105c` migró reenvío de invitaciones y `d1724e1` migró activar/desactivar. En ese corte, **23 de las 41 combinaciones** usaban controllers MVC y la suite backend pasó **1489/1489, sin omitidos**. Este párrafo registra el avance intermedio, no el estado más reciente.

**Checkpoint estructural de 41/41 (2026-09-24, hasta `a9f2a8e`):** las 18 combinaciones restantes se sustituyeron por controllers MVC, incluido `/connect` y el webhook de WhatsApp. El inventario mantiene **41/41** combinaciones explícitas, con las cuatro condicionales registradas solo cuando corresponde; OpenIddict y Aspire conservan sus endpoints técnicos. Los workers de WhatsApp usan servicios de Application y la operación EF de retención quedó en un repositorio especializado. Los helpers y modelos productivos salieron de `Application/Features`; `IdentityService` delega lecturas de negocio en lectores/repositorios. Tras integrar rutas, workers y traslado de `Features`, la solución compiló con **0 advertencias y 0 errores** y la suite backend completa pasó **1504/1504, sin omitidos** en `847b8de`. Los cambios posteriores de nombre, carpetas de tests e Identity compilaron; las 439 pruebas unitarias de Application y las 20 focalizadas de Identity pasaron. `149e451` retiró `IEndpoint`, `MapEndpoints`, handlers, decoradores, `Application/Abstractions` y Scrutor; sobre la integración en ese commit, el build pasó sin advertencias/errores y la suite backend pasó **1489/1489, sin omitidos**. Luego `23b8337` retiró `RouteHandlerOptions` de Minimal API; `4592600` eliminó un adaptador `IResult` sin consumidores y modelos de invitación duplicados, y repuso logs de inicio/fin del webhook; `fcdd9f5` movió el lector y repositorio de ajustes a `Infrastructure/Persistence`. Tras `fcdd9f5`, la integración compiló con **0 advertencias y 0 errores** y el filtro HTTP de Ajustes, webhook y binding MVC pasó **95/95, sin omitidos**. `a9f2a8e` integró la transacción de ingreso Google: en la rama de origen pasaron build 0/0, 10 pruebas unitarias ExternalLogin y 22 HTTP de ExternalLogin/RegistrationMode con Testcontainers; un test fuerza un fallo después del autoguardado de Identity y comprueba rollback de usuario, rol, vínculo y auditoría, con cookie solo después del commit. Sobre la integración final en `a9f2a8e`, `dotnet build ArquitecturaBase.slnx` pasó con **0 advertencias y 0 errores** y `dotnet test ArquitecturaBase.slnx` pasó **1491/1491, sin omitidos**, incluidos los tests de migraciones y seed con bases temporales. El frontend se verificó en un worktree aislado de `ArquitecturaBaseFront` (`e2df264`): `npm run build`, `npm run lint` y `npm run test` verdes, **632/632 pruebas**. No se ejecutó una prueba de navegador contra la API migrada ni una validación manual de Aspire/OIDC/WhatsApp sobre esta rama.

**Puertas abiertas para el cierre:** probar los flujos principales con frontend/API migrados y verificar manualmente Aspire, OIDC y Meta/WhatsApp; revisar el arranque real de la rama aislada con una base vacía. El build y las suites backend/frontend automatizadas están verdes. Los tests de base vacía prueban migraciones y seed en bases temporales, pero no sustituyen un arranque manual de la aplicación. Para ese smoke manual se preparó el worktree frontend hermano `../ArquitecturaBaseFront` en el directorio de la integración (`e2df264`, dependencias instaladas con `npm ci --offline`); Aspire aún no se inició allí.

**Historial de cortes y puertas pendientes:** cada familia se integró secuencialmente en `codex/mvc-migracion` con su inventario de rutas y pruebas de área. La automatización final está verde; los flujos manuales siguen la fila 9 y las puertas del final del plan.

| Orden | Corte | Estado de preparación | Puerta específica antes del siguiente corte |
|---|---|---|---|
| 1 | `POST /account/login-code` → `LoginCodeController`/`IAccountService` | Integrado en `da5c801`; el controller se renombró en `d6ef8fb`; su handler viejo se retiró | `LoginCodeEndpointsTests`, rate limits, contrato Auth, inventario 41 y arquitectura pasaron. |
| 2 | `POST/PUT/DELETE /api/roles` → `RolesController`/`IRoleService` | Integrado en `b8bb5d9`; sus handlers viejos se retiraron | `RoleCrudEndpointsTests`, rollback de claims, políticas `roles.manage`, OpenAPI e inventario 41 pasaron. |
| 3 | `POST /account/login-code/verify` → `LoginCodeController`/`IAccountService` | Integrado en `9ff4d80`; el controller se renombró en `d6ef8fb`; su handler viejo se retiró | Login por correo/WhatsApp, auditoría y guardado en error, cookie, privacidad de logs, rate limit e inventario 41 pasaron. |
| 4 | `POST /api/users` y `PUT /api/users/{id:guid}` → `UsersController`/`IUserService` | Integrado en `05ff70c`, con repositorio real `69e17ab`; handlers viejos retirados | Creación/edición, Identity, invitaciones, locks, rollback y contratos HTTP pasaron. |
| 5 | `GET/PUT /api/me` y `POST /account/login-code/whatsapp` | Integrados en `26174f6`, `c0f8d97` y `bb53126`; handlers viejos retirados | Perfil, privacidad de logs, rate limit compartido y ruta WhatsApp ausente con función apagada pasaron. |
| 6 | Cinco rutas de usuarios restantes: invitación, activar, desactivar, borrar y desvincular WhatsApp | Integradas en `UsersController`; helpers productivos reubicados en `Application/Services/Users`. | Contratos HTTP y pruebas de usuarios incluidos en la suite backend de 1504/1504. |
| 7 | Cinco rutas de perfil restantes: código/vinculación/desvinculación WhatsApp y código/confirmación de correo | Integradas en `MeController`; persistencia en error, locks y registro condicional conservados. | Contratos HTTP y pruebas de perfil incluidos en la suite backend de 1504/1504. |
| 8 | Dos rutas externas de Google, siete de `/connect` y dos del webhook | Integradas en `ExternalLoginController`, `ConnectController` y `WhatsAppWebhookController`; workers migrados a servicios. | Contratos Auth/OIDC/WhatsApp e inventario 41 incluidos en la suite backend de 1504/1504. |
| 9 | Retiro de `Features`, handlers, `IEndpoint` y decoradores restantes | Limpieza integrada en `149e451`; quedan la suite final sobre la integración y las comprobaciones manuales. | Build, backend/frontend, base vacía automatizada, flujos manuales de ingreso/OIDC/WhatsApp y revisión final. |

`main` no recibe estos commits hasta pasar las puertas del área, la suite backend/frontend completa y los flujos manuales de cierre. El estado de Visual Studio sobre el checkout principal se conserva durante la ejecución aislada.
...
Se eligió **usar la solución y los proyectos existentes**, reescribiendo `Api` y `Application` por áreas con la arquitectura acordada. La base descartable elimina cualquier tarea de traslado de datos o compatibilidad con el historial de migraciones. Un proyecto nuevo habría exigido volver a conectar el frontend, Identity, OpenIddict, el webhook, los workers, AppHost y las pruebas; la reescritura interna permitió probar cada funcionalidad antes de retirar su endpoint y handler anteriores. No se agregó otro repositorio ni otro `.csproj` por este cambio de arquitectura.
...
### Situación inicial y estructura final

Al redactar el plan, el recorrido era `Api/Endpoints → Application/Features/<CasoDeUso>/*Handler → interfaces de Domain o Application/Abstractions → Infrastructure`. Los handlers cumplían la función de servicios de aplicación, pero su nombre y dispersión por caso de uso no mostraban el recorrido convencional. `Api` y `Application` ya estaban libres de `ApplicationDbContext`, EF, `IQueryable` y `UserManager`; había que encapsular lecturas EF de negocio en `Infrastructure/Identity/IdentityService` y `PermissionService`, además de la operación de retención de WhatsApp. El recorrido resultante es `Api/Controllers → Application/Interfaces/Services → Application/Services → Application/Interfaces/{Persistence,Integrations} → Infrastructure`, sin reescribir las reglas de negocio que funcionan.
...
`Interfaces` es una **carpeta de contratos dentro de Application**, no un proyecto adicional. Los siete contratos `I*Repository` que estaban junto a agregados de `Domain` y los contratos de `Application/Abstractions` se trasladaron en cambios mecánicos: interfaces de servicio a `Interfaces/Services`, repositorios/lectores a `Interfaces/Persistence`, puertos externos a `Interfaces/Integrations`, y records/DTO a `Models`. Las implementaciones están en `Services` o `Infrastructure`; `IAuditable`, `ISoftDeletable` y otras interfaces propias del modelo permanecen en `Domain`. No crear un `IRepository<T>` genérico ni una interfaz por cada clase técnica.
...
3. Los contratos consumidos por servicios viven en `Application/Interfaces`; las implementaciones de repositorios y lectores viven en `Infrastructure`. Los siete contratos de repositorio antes ubicados en `Domain` se movieron mecánicamente a `Application/Interfaces/Persistence`, sin cambiar firmas. `Domain` conserva el modelo y sus propias interfaces internas. No se expone `IQueryable`, `ApplicationUser` ni `ApplicationRole` fuera de Infrastructure.
...
- [x] Capturar baseline: `dotnet build ArquitecturaBase.slnx` y `dotnet test` en backend; `npm run build`, `npm run lint` y `npm run test` en `../ArquitecturaBaseFront`. Los resultados iniciales, la falla previa del arnés y el control posterior del frontend aislado están registrados en los checkpoints de arriba; los tests de integración usaron Docker.
...
- [x] Integrar un commit por área con sus pruebas focalizadas y comprobar el inventario tras cada corte. La rama conserva las 41 combinaciones explícitas sin rutas duplicadas; el resultado completo previo a la limpieza fue 1504/1504.
...
- [x] Implementar el adaptador MVC usando `ProblemDetailsMapper`, conservando status, `code`, `traceId`, errores de validación e idioma. `ControllerResultExtensions` quedó en `4aa87c1` con 3/3 pruebas HTTP verdes; `ResultExtensions` se conservó durante la coexistencia transitoria con Minimal API.
...
- [x] Definir y aplicar logging en los servicios de caso de uso con `ILogger<TService>` y `[LoggerMessage]`: inicio, resultado y código de error donde corresponde, sin datos sensibles. `SystemSettingsService` y sus pruebas fijaron el patrón en `76702e2`; la auditoría transversal posterior al retiro de `LoggingDecorator` quedó registrada en la Tarea 8.
...
- [x] Mapear controllers en `Program.cs`; durante la transición convivió `MapEndpoints()` para las áreas aún no migradas. `ExplicitRouteInventoryTests` confirmó las 41 combinaciones sin rutas nuevas ni duplicadas; `MapEndpoints()` se retiró en `149e451`.
- [x] Correr los tests de error, localización, OpenAPI y arquitectura. `MvcConnectPassthroughTests`, `MvcBindingContractTests`, OpenAPI y la suite backend verifican los bordes HTTP, 404/405/415 y el orden de middleware; el build fue 0/0 y la suite 1384/1384 en el primer piloto, con resultados posteriores registrados arriba.
- [ ] Comprobar manualmente Aspire y el SPA con la API migrada desde el worktree, sin alterar la instancia que corre desde Visual Studio.
...
- [x] Probar las lecturas trasladadas desde `IdentityService` y `PermissionService`: `IdentityServiceTests`, `PermissionServiceTests`, `RoleReaderTests` y contratos HTTP cubren soft delete, roles ordenados, filtros, paginado, normalización, proyección y caché, incluida la reasignación de roles sin invalidar permisos cacheados.
- [x] Extraer las consultas EF de negocio de `IdentityService` y `PermissionService` a lectores especializados; conservar filtros de cuentas borradas, conteos y caché. Las interfaces no retornan `IQueryable`, `ApplicationUser` ni `ApplicationRole`. `fc858f8` retiró las lecturas residuales de roles/usuarios de `IdentityService`.
- [x] En `IdentityService.RevokeSessionsAsync`, trasladar la consulta directa a `dbContext.LoginLinks` al `ILoginLinkRepository`. `ListPendingAsync` incluye enlaces **sin consumir y sin invalidar aunque ya hayan expirado**, mantiene tracking y el mismo contexto para guardar antes de `UpdateSecurityStampAsync`. La nueva prueba revoca un enlace vencido; las suites de enlaces y backend pasan. El resto del adaptador técnico Identity sigue en Tarea 3.
- [x] Encapsular escrituras de usuarios/roles en repositorios o adaptadores de Identity, con una responsabilidad clara por operación; `IIdentityService` no se convirtió en un envoltorio de `IUserRepository`. Los actos técnicos de sesión, cookie, seguridad y revocación de OpenIddict permanecen en Infrastructure. No se creó un repositorio genérico `IRepository<T>` ni repositorios para SMTP o HttpClient.
- [x] Mantener `ISystemSettingsReader` como lector especializado. `UnitOfWork`, migraciones, seeders, health checks y stores del framework siguen como infraestructura técnica, sin repositorios ficticios.
- [x] Probar el límite transaccional compartido de usuarios y roles: `UserRepositoryTransactionTests` cubre fallos de invitación, contacto, revocación, borrado y desvinculación; `RoleRepositoryTransactionTests` cubre fallo de claims; pruebas de servicios Auth y perfil cubren guardado exigido aun en error.
- [x] Comprobar con un test de integración que un fallo intermedio en el ingreso Google revierte alta, rol, enlace y auditoría después del autoguardado de Identity. `a9f2a8e` integra la transacción y deja la cookie para después del commit; en la rama de origen pasaron 10 unit y 22 HTTP con Testcontainers.
- [x] Registrar contratos e implementaciones scoped en `Infrastructure/DependencyInjection.cs`. Las suites de Identity, permisos, usuarios y roles se ejecutaron durante los cortes de esas áreas.
...
- [x] Pasar los casos de uso implementados a `IUserService`, `IProfileService` e `IRoleService`: crear/editar/restaurar/desactivar/borrar usuario, filtros/conteos, invitaciones, desvinculación administrativa, perfil con correo y WhatsApp, y roles/permisos. Los contratos HTTP comprueban `lastInvitation.deliveryStatus`, `WaMessageId`, números formateados/enmascarados y estados de verificación.
- [x] Probar reglas de usuario/rol en servicios y por HTTP: cuenta borrada, último Admin, permisos, filtros y conteos, concurrencia e invalidación de permisos. Para perfil, las pruebas de `ConfirmEmail` y `ConfirmPhoneLink` cubren el guardado exigido aun cuando la verificación o unicidad devuelve error; los servicios lo hacen explícitamente sin `IPersistChangesOnFailure`.
- [x] Conservar `PhoneNumberChange`, `WhatsAppContactLinker` y `DestinationCodeVerifier`: locks en orden contactos → cuenta, códigos `VerifyDestination` por cuenta, revocación de sesiones y enlaces al soltar un número. `MeWhatsAppEndpointsTests`, `MeEmailEndpointsTests`, `UserInvitationEndpointsTests`, `UnlinkUserPhoneEndpointTests` y `WhatsAppLockOrderTests` integran la regresión automatizada.
- [x] Mover lógica de handlers a servicios, conservando helpers de negocio (`UserGuards`, etc.). Los servicios coordinan validadores y `IUnitOfWork`; las consultas no guardan y las escrituras preservan el guardado en error donde corresponde.
- [x] Crear controllers con rutas y permisos idénticos. Se conservaron `/{id:guid}`, defaults de paginado, filtros y cuerpos JSON; las familias se sustituyeron secuencialmente.
- [x] Después de cada familia se ejecutaron pruebas del área y el contrato HTTP; se retiraron sus endpoints y handlers anteriores sin duplicar rutas.
...
- [x] Probar los flujos donde un error **sí** debe persistir intento, auditoría o consumo: verificación de código, canje de enlace e ingreso externo. El guardado explícito en servicios reemplazó `IPersistChangesOnFailure`; los tests de servicio y HTTP cubren los casos de error.
- [x] Migrar código y enlaces a servicios con validación explícita antes de la operación; conservar locks, cuotas, expiración, idioma, emisión y respuesta HTTP. Se conservaron `202` en pedidos de códigos/invitaciones, `204` y cookie al canjear enlace, `415` para cuerpos no JSON y la partición compartida de rate limits `login-code`/`login-verify` con perfil.
- [x] Mantener `Challenge`, callback y redirección Google en el controller/adaptador de presentación; el servicio de Application decide reglas de cuenta sin `HttpContext`. Las pruebas cubren cuentas sin correo, errores y `returnUrl` de Google al SPA.
- [x] Cambiar endpoints por controllers en familias separadas, preservando rutas `/account/*`, status, JSON, cookies y las políticas `login-code`/`login-verify`.
- [x] Ejecutar pruebas de autenticación y contrato HTTP tras cada familia.
...
- [x] Antes de sustituir rutas, probar passthrough OpenIddict en un controller MVC: GET/POST authorize, POST token con formulario, GET/POST logout/userinfo, challenge, sign-in, forbid y redirecciones (`MvcConnectPassthroughTests`).
- [x] Conservar issuer usado por el SPA, PKCE, scopes, cookies, revocación de tokens, carga de principal y ausencia de permisos en el token. Las pruebas de integración verifican `/connect/revoke`, `/connect/introspect` y discovery `/.well-known/*`, que administra OpenIddict, y la exclusión de `/connect` del OpenAPI público.
- [x] Cambiar las cuatro familias de `/connect` (siete combinaciones verbo/ruta) en `ConnectController` después de la prueba de protocolo y la suite de integración, sin dos handlers para el mismo verbo/ruta.
...
- [x] Conservar la ausencia de `GET+POST /webhooks/whatsapp` si `IsWebhookEnabled` es falso, y la ausencia de los dos `POST` condicionales de cuenta/perfil si `IsEnabled` es falso. El registro MVC condicional se prueba en startup; `PUT+DELETE /api/me/whatsapp` y login link siguen mapeados aun con WhatsApp apagado.
- [x] Leer el cuerpo crudo en el controller antes de deserializar; respetar límite de 5 MB aun sin `Content-Length`, firma, challenge `text/plain`, 401/413 y 200 vacío para eventos firmados.
- [x] Pasar el lote validado al servicio. `WhatsAppWebhookController` solo adapta HTTP; `WhatsAppWebhookService` coordina idempotencia y un reintento tras `UniqueConstraintViolationException`; el adaptador de Infrastructure crea un scope nuevo. Las pruebas HTTP cubren el conflicto concurrente, orden de locks, señal posterior al commit y estados.
- [x] Hacer que los workers de entrada y envío invoquen servicios de Application, conservando señal **y** polling, scope y transacción por contacto, fallos aislados, cola acotada, cancelación y logs sin secretos. El sender conserva tres intentos, registra saliente o fallo y no reenvía si solo falló guardar el historial; se conserva encolar antes del commit.
- [x] Mantener `WhatsAppMessageRetentionService` como scheduler técnico en Infrastructure, también con WhatsApp apagado; la operación EF que borra `Body` está en `IWhatsAppMessageRetentionRepository`/`WhatsAppMessageRetentionRepository`. Las opciones permanecen en Infrastructure, sin servicio de Application nominal.
- [x] Correr la suite WhatsApp y pruebas con feature apagada/prendida antes de retirar endpoints y handlers anteriores; el suite HTTP focalizado pasó 295/295 tras integrar el servicio inbound.
...
- [x] Sustituir los endpoints de prueba de `ApiFactory` por controllers MVC de prueba con un `ApplicationPart` selectivo; conservar los tests de errores y binding. Las pruebas de decoradores viejos se retiraron tras migrar validación y commits/rollback a pruebas de servicios y repositorios (`149e451`).
- [x] Antes de retirar `AddFeaturesFromAssembly`, comprobar por DI que todos los validadores de Application se registran y resuelven por scope; `DependencyInjectionTests` cubre además el rechazo de un request inválido y las pruebas de servicios verifican que no llega a guardar.
- [x] Revisar los logs operativos de todos los métodos públicos migrados: Account, LoginLink, ExternalLogin, Roles, Settings, Users, Profile, Delivery e Inbound conservan inicio y resultado/error; `4592600` repuso esos logs en el webhook incluso para un lote firmado vacío. La revisión estática complementa las pruebas de logging y privacidad existentes; no se agregó una prueba universal nueva.
- [x] Buscar referencias a `IEndpoint`, `ICommandHandler`, `IQueryHandler`, `AddFeaturesFromAssembly` y `MapEndpoints` en `src` y `tests`; el pipeline sin consumidores se retiró en `149e451`.
- [x] Mover los helpers y modelos productivos de `Application/Features/` a `Services/<Area>/`, `Models/<Area>/`, `Configuration/` o `Validation/<Area>/`, en cambios separados del comportamiento. La limpieza `149e451` retiró también `Application/Abstractions`.
- [x] Mover helpers de OpenIddict retenidos a `Api/Authentication/` y quitar Scrutor del proyecto Application y del catálogo de paquetes (`149e451`); la DI registra servicios de forma explícita.
- [x] Reconciliar `AGENTS.md`, `CLAUDE.md`, `README.md` y la especificación con la estructura MVC final, conservando en este plan el historial y las puertas todavía pendientes.
- [x] Ejecutar `dotnet build ArquitecturaBase.slnx` sin warnings y `dotnet test ArquitecturaBase.slnx` completo con Docker sobre `a9f2a8e`: **0 advertencias/errores y 1491/1491 pruebas, sin omitidos**. En el worktree frontend `e2df264`, build, lint y tests pasaron **632/632**.
- [x] Revisar el diff, la arquitectura y el inventario HTTP automatizado: 41 combinaciones explícitas sin duplicados; ningún controller inyecta persistencia, `Api`/`Application` no referencian EF, no quedan rutas de negocio Minimal API ni handlers de producción. Los contratos HTTP automatizados y la revisión final no detectaron diferencias observables.
- [ ] Comprobar manualmente el arranque de la API migrada con una base de desarrollo nueva y los flujos con el frontend real: Visual Studio/Aspire, login por correo y WhatsApp, invitaciones, vincular/desvincular desde Mi perfil, administración, `/connect`/OIDC y webhook Meta. Los tests ya cubren migraciones y seed en bases temporales; esta puerta verifica la aplicación levantada. Documentar cualquier diferencia de esquema al recrear la base.
