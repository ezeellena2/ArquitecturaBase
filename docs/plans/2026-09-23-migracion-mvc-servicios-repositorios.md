...
**Avance de la Tarea 1:** se movieron los siete contratos de repositorio y todos los contratos retenidos de `Application/Abstractions`; allí solo quedan `Messaging` y `Behaviors` durante la transición. `AddControllers` y `MapControllers` ya conviven con las rutas existentes, sin duplicarlas. El adaptador MVC de `Result` conserva 200/204 y `ProblemDetails` en tres pruebas HTTP; las pruebas de arquitectura fijan los límites nuevos. La solución compila con 0 advertencias y 0 errores, y la suite backend completa pasó **1356/1356** después de integrar Identity y WhatsApp. Todavía no se sustituyó ninguna ruta de negocio.
...
- [ ] Configurar las respuestas de binding/model state para conservar `Request.Invalid`; `RouteHandlerOptions.ThrowOnBadRequest` no configura MVC.
