...
**Checkpoint de persistencia y cuenta:** `238658c` encapsuló los enlaces pendientes (incluidos los vencidos) para revocar sesiones; `037a9b9` separó la lectura fresca de roles del usuario y los permisos cacheados por rol; `5bd6a4f` trasladó filtros, listado y conteos de usuarios a `UserReader`; `06530fd` trasladó nombres, listado, detalle y comprobación de nombres de roles a `RoleReader`. En paralelo, `6d9a6c5` migró `GET /account/login-methods` a `LoginMethodsController`/`IAccountService`, retirando su handler anterior. La solución compiló con **0 advertencias y 0 errores**, el inventario sigue en **41** verbo/ruta y la suite completa pasó **1392/1392**. Quedan las demás búsquedas de usuario, las escrituras y los límites transaccionales de Identity antes de cerrar la Tarea 3. Las rutas GET de usuarios y roles se están preparando en worktrees separados para la siguiente integración.

**Checkpoint actual (2026-09-24, rama `codex/mvc-migracion`):** `b5a1d63` amplió `IUserReader` y `IdentityService` delega esas lecturas; `6d9a6c5`, `3e1dea6` y `30106c9` migraron login methods y login link; `bde0ed4` y `06d4417` migraron roles/permisos de lectura; `a8d0c9d`, `8654157` y `b8ccbd4` migraron las tres lecturas de usuarios. `c8659d4` preparó roles con `IRoleRepository` transaccional y `b8bb5d9` cambió sus escrituras; `fcd5934`/`da5c801` migraron el pedido de código; `ac3984f`/`9ff4d80` migraron la verificación y corrigieron una fuga de código/número en logs automáticos de MVC. `a140dc7` preparó alta/edición de usuarios, `69e17ab` extrajo `IUserRepository` real con dos pruebas de rollback y `05ff70c` cambió las dos rutas. `26174f6`/`45817d8`/`c0f8d97` migraron GET/PUT de perfil; `9d5c6a1`/`bb53126` migraron el pedido WhatsApp con ausencia real de la ruta si la función está apagada. `5a84f9b` preparó el servicio Google y `29f23af` los servicios de correo del perfil; `f93105c` migró reenvío de invitaciones y `d1724e1` migró activar/desactivar. **23 de las 41 combinaciones** ya usan controllers MVC; las otras 18 siguen atendidas por la implementación anterior. La solución compiló con **0 advertencias y 0 errores** y la suite backend completa pasó **1489/1489, sin omitidos** sobre este estado. No se tocó `main`, el frontend ni la instancia de Aspire de Visual Studio.

**Próximos cortes concretos:** cada fila se integra en `codex/mvc-migracion` con un commit de sustitución de rutas, se elimina el handler anterior de esa familia y se verifica paridad de verbo/ruta, autorización, rate limit, JSON/status/errores, transacción, OpenAPI y pruebas de área. Las preparaciones en worktrees de agentes no activan rutas duplicadas.

| Orden | Corte | Estado de preparación | Puerta específica antes del siguiente corte |
|---|---|---|---|
| 1 | `POST /account/login-code` → `AccountController`/`IAccountService` | Integrado en `da5c801`; su handler viejo se retiró | `LoginCodeEndpointsTests`, rate limits, contrato Auth, inventario 41 y arquitectura pasaron. |
...
| 3 | `POST /account/login-code/verify` → `AccountController`/`IAccountService` | Integrado en `9ff4d80`; su handler viejo se retiró | Login por correo/WhatsApp, auditoría y guardado en error, cookie, privacidad de logs, rate limit e inventario 41 pasaron. |
...
| 6 | Cinco rutas de usuarios restantes: invitación, activar, desactivar, borrar y desvincular WhatsApp | Invitación y activar/desactivar integradas en `f93105c`/`d1724e1`; borrar y desvincular están en un worktree de área | Último Admin, invitaciones, revocación, locks, soft delete, UoW/rollback y contratos HTTP. |
| 7 | Cinco rutas de perfil restantes: código/vinculación/desvinculación WhatsApp y código/confirmación de correo | Servicio de correo preparado en `29f23af`; corte HTTP de correo en worktree de área; WhatsApp pendiente | `IPersistChangesOnFailure`, sesión, locks, teléfono/correo, 400/415 y rutas condicionales. |
| 8 | Dos rutas externas de Google, siete de `/connect` y dos del webhook | Servicio Google preparado en `5a84f9b`; corte Google y preparación Connect en worktrees separados; webhook pendiente | Callback/returnUrl, passthrough OpenIddict, firma y cuerpo crudo Meta, workers, inventario 41. |
| 9 | Retiro de `Features`, handlers, `IEndpoint` y decoradores restantes | Solo cuando las 18 rutas y workers estén migrados | Build, suite backend/frontend, base vacía, flujos manuales de ingreso/OIDC/WhatsApp y revisión final. |
