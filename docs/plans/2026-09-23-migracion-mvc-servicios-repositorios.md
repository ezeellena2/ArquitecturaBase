...
**Checkpoint actual (2026-09-24, rama `codex/mvc-migracion`):** `b5a1d63` amplió `IUserReader` para búsquedas de cuentas y `IdentityService` delega esas lecturas; `6d9a6c5`, `3e1dea6` y `30106c9` migraron login methods y login link; `bde0ed4` y `06d4417` migraron las lecturas de roles/permisos; `a8d0c9d`, `8654157` y `b8ccbd4` migraron las tres lecturas de usuarios. `c8659d4` preparó roles con `IRoleRepository` transaccional y `b8bb5d9` cambió sus tres rutas de escritura; `fcd5934` preparó el pedido de código y `da5c801` cambió su ruta HTTP. **14 de las 41 combinaciones** ya usan controllers MVC; las otras 27 siguen atendidas por la implementación anterior. Tras esos cortes, la solución compiló con **0 advertencias y 0 errores** y la suite backend completa pasó **1413/1413, sin omitidos**. `ac3984f` preparó verificación de códigos y `a140dc7` alta/edición de usuarios, ambos sin cambiar aún sus rutas; el build y las 413 pruebas unitarias de Application pasaron después de integrarlos. Falta repetir la suite completa con esos dos servicios incorporados. No se tocó `main`, el frontend ni la instancia de Aspire de Visual Studio.
...
| 3 | `POST /account/login-code/verify` → `AccountController`/`IAccountService` | Servicio listo en `ac3984f`; corte HTTP en worktree separado | Login por correo/WhatsApp, auditoría y guardado en error, cookie, privacidad de logs, rate limit e inventario 41. |
| 4 | `POST /api/users` y `PUT /api/users/{id:guid}` → `UsersController`/`IUserService` | Servicio listo en `a140dc7`; `IUserRepository` y rollback intermedio pendientes antes del corte HTTP | Creación/edición, Identity, permisos, invitaciones, locks y contratos HTTP. |
| 5 | Resto de cuenta, usuarios y perfil | Pendiente, por familias independientes | Tests Auth/Users/Me, contratos de frontend, errores que deben guardar y rutas condicionales. |
| 6 | `/connect`, WhatsApp y retiro del pipeline viejo | Pendiente de servicios y pruebas de protocolo/feature | OpenIddict, webhook crudo/firma, workers, suite completa y revisión manual con el front. |
