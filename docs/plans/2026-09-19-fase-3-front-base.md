...
8. **OIDC en el SPA** (`oidc-client-ts` 3.5.0 con `react-oidc-context` 3.3.1):
   ...
   - `stateStore` **va explícito en `sessionStorage`**. Ahí viaja solo el `state` y el `code_verifier` de la transacción, nunca tokens; si fuera en memoria, el ida y vuelta a `/connect/authorize` perdería el estado y el callback fallaría. **Corrección (2026-09-19, verificada en el código de la librería):** el default de `stateStore` **no** es `sessionStorage` sino `window.localStorage` (`OidcClientSettingsStore`); el que defaultea a `sessionStorage` es `userStore`. O sea: los dos overrides son necesarios, y por motivos distintos. Sin el de `userStore`, los tokens irían a `sessionStorage`, que es lo que el spec prohíbe; sin el de `stateStore`, el `code_verifier` quedaría en `localStorage`, que sobrevive a la pestaña.
...
11. **`/login/codigo` sin `returnUrl` deja el ingreso en un callejón sin salida.** Quien abra esa URL a secas (tipeada, un favorito viejo, un link compartido) cae en `returnUrl = "/"`; el guardia navega a `/login?returnUrl=%2F`; `LoginPage` ve un `returnUrl` *truthy* y por eso **no** arranca el OIDC, así que muestra el formulario; y al verificar, el backend rechaza `"/"` con `ReturnUrls.IsAuthorizeRequest`. El usuario queda en un formulario que nunca va a funcionar y no tiene cómo salir sin editar la URL. Por el botón de Google es peor: `/account/external/google?returnUrl=%2F` devuelve el ProblemDetails como página JSON cruda. El arreglo es tratar un `returnUrl` que no sea una ruta de `/connect/authorize` como si no estuviera, y navegar a `/login` **sin query**, que es lo que reinicia el flujo.
   ...
   **Resuelto el 2026-09-20** (`b07d03f`): sin `returnUrl` en la query, `LoginCodePage` manda a `/login` sin parámetro. Lo cubre un test que comprueba que no se muestra el formulario del código y que arranca el redirect de OIDC.
12. **Un fallo transitorio de `/api/me` se confunde con falta de permiso.** `usePermissions` colapsa "todavía no sé", "falló la consulta" y "no tiene permisos" en `permissions: []`. Si `/api/me` devuelve 500, `isPending` pasa a `false` sin datos, `has("users.read")` da `false` y `ProtectedRoute` manda a `/sin-permiso`. Y no hay reintento que lo tape: la política de `queryClient` no reintenta un `ApiError` que no sea de red. Es la peor clase de error, porque diagnostica mal: le dice al usuario que no tiene permiso cuando lo que pasó es que el backend se cayó. En el mismo escenario el sidebar esconde todo el menú y el tablero queda en blanco. El arreglo es exponer `isError` y mostrar un estado de error con reintento.
    ...
    **Resuelto el 2026-09-20** (`6922383`): `usePermissions` expone `isError`, `error` y `refetch`, y `ProtectedRoute` muestra el mismo estado de error que `DataTable` —con el `traceId` y un botón para reintentar— en vez de negar el permiso. Dos tests lo cubren: que un 500 no dice "no tenés permiso", y que el botón de reintentar entra a la ruta cuando la segunda respuesta llega bien.
...
15. **Tests que no prueban lo que dicen.** Cuatro, y los cuatro pasarían con el código roto:
    ...
    **Resuelto el 2026-09-20** (`3387c2b` y `235fd76`): `ProtectedRoute.test.tsx` monta la ruta protegida con permiso y cubre las dos ramas, con `/sin-permiso` en su router; se comprobó el rojo-verde (forzando `has()` a `true`, el test de la redirección falla). `usePermissions.test.tsx` espera a que la consulta resuelva antes de afirmar la ausencia. El test de i18n muestra una clave que sí cambia entre idiomas. La aserción de `JSON.stringify(authConfig)` se sacó: las sondas que escriben en cada store, que sí dan la garantía, ya estaban.
...
18. **Claude Code arma sus worktrees dentro del repo.** Las copias que deja en `.claude/worktrees/` son proyectos enteros con sus propios tests, y Vitest los recogía: `npm run test` terminaba con decenas de fallos de otra sesión, en archivos que uno no tocó. **Resuelto el 2026-09-20** (`abab8cb`): `vite.config.ts` excluye `.claude/**` de la corrida. Si aparece otra herramienta que anide copias del proyecto, va a hacer falta lo mismo.

19. **Dos desvíos menores del spec, no declarados:** en la sección 7.2, los grupos del menú lateral tenían que desplegarse y quedaron como rótulos estáticos; y la barra superior tenía que llevar migas **y título de la página**, y el título vive en `PageHeader`, dentro del contenido.
