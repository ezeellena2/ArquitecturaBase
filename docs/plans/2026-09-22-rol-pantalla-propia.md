...
| `picker.pickAllFor` | Elegir todos los permisos de {{area}} | Pick all permissions in {{area}} |
| `picker.clearAll` | Quitar todos | Remove all |
| `picker.clearAllFor` | Quitar todos los permisos de {{area}} | Remove all permissions in {{area}} |
...
3. **Cuerpo:** un `<form id noValidate>` con dos columnas (`340px` + el resto, 16 px entre columnas, `items-start`; en menos de `lg` se apilan). El Guardar de la banda es `type="submit" form={id}`.
   - **Izquierda, adherida al scrollear** (`lg:sticky`, con `top` igual a la banda más el padding del cuerpo: `lg:top-[calc(3.5rem+1.5rem)]`; en la revisión final pasó a `lg:top-14`, pegada a la banda al scrollear, como en el tablero):
...
- [x] **Comparar con el tablero**, incluida la columna izquierda adherida al scrollear en un navegador, y anotar en el commit cualquier diferencia que quede a propósito.
  ...
  >
  > **Diferencias que quedan a propósito**, todas de piezas compartidas: la barra superior mide 64 px y no 60; el título de la banda, 18 px y no 16 (`Page`); el cuerpo, 24 px de padding arriba y no 20 (`Page`), así que antes de scrollear la columna queda 24 px debajo de la banda y no 20 (al scrollear, en cambio, se adhiere pegada a la banda, como en el tablero: ver “Revisión final”); y los rótulos de los campos, 14 px y no 13 (`FormField`), con lo que “Datos del rol” mide 241 px y no 246. Después de la revisión se suman tres, también de piezas compartidas: el asterisco de “Nombre *” sale del color del rótulo y no rojo (`FormField`); el código para reportar va como texto y no con estilo `code` (`EmptyState` recibe la descripción como texto, y así la muestran todas las pantallas); y el rol que ya no existe y el error de carga usan el recuadro punteado de `EmptyState`, no una tarjeta blanca. En la revisión final, dos más de `FormField`: la ayuda y el error debajo de un campo miden 14 px y no 12,5 (“Poné un nombre.”, “Los roles del sistema no cambian de nombre.”), y el rótulo de un campo de solo lectura no se pinta en gris como en la tarjeta de User. Alinearlas es cambiar la pieza compartida y su biblioteca, en una tarea aparte. Las áreas “de ejemplo” no existen, y con “Elegidos” de dos cifras (Admin) “Expandir todo” y “Contraer todo” bajan de renglón. También se vieron la carga, Admin, User con cambios, “¿Descartar los cambios?” y el guardado que vuelve al listado sin preguntar.
...
3. **Lo que la biblioteca ArquitecturaBase UI tendría que reflejar** de este plan, cuando la regla de llevar allí cada cambio de aspecto se confirme (hoy es un cambio sin commitear de otra sesión, que es la dueña de la biblioteca y del lienzo):
   ...
   - `AppLayout` (en la biblioteca, el caparazón): la raíz en `h-svh`, la columna con `min-h-0`, y `main` `relative`, con `min-h-0`, `overflow-y-auto` y `scroll-pt-14` (el alto de la banda, para que no tape el foco); al imprimir, `print:h-auto` en la raíz y `print:overflow-visible` en `main`.
...
5. **Diferencias a propósito con el tablero que son de piezas compartidas** (Tarea 7): la barra superior mide 64 px y no 60; el título de la banda, 18 px y no 16; el padding del cuerpo, 24 y no 20; los rótulos de campo, 14 px y no 13, con el asterisco del color del rótulo; la ayuda y el error debajo de un campo, 14 px y no 12,5, y el rótulo de un campo de solo lectura, del color de siempre y no en gris (los dos de `FormField`, así que se ven en todos los formularios); el código para reportar va como texto; y el vacío y el error usan el recuadro punteado de `EmptyState`. Alinearlas es cambiar esas piezas y su biblioteca.
...
7. **Las teclas de scroll con `main` como el que scrollea** (de la revisión final, ver abajo). Re Pág, Av Pág, Espacio y las flechas mueven `main` recién cuando el foco o un clic están adentro: al llegar a una pantalla desde el menú, o al recargarla con el foco en `<body>`, no hacen nada. Antes scrolleaba el documento y andaban desde cualquier lado. Hay dos caminos y los dos son una decisión: volver a que scrollee el documento con la barra superior y la banda adheridas (`sticky top-0` y `sticky top-16`, con `scroll-padding-top` en `html`), que cambia el caparazón, `Page` y su biblioteca y la regla de `AppLayout` del `CLAUDE.md` del front; o darle a `main` `tabIndex={-1}` y enfocarlo al cambiar de ruta, que cambia a dónde va el foco en cada navegación.

### Revisión final

Una revisión de la feature entera (el recorrido, los tableros, accesibilidad, textos, tests y regresiones) confirmó doce hallazgos, dos de ellos el mismo (la banda que tapa el foco). Se arreglaron diez en el front, cada uno en su commit y, los que cambian comportamiento, con el test visto en rojo primero; otro quedó documentado y uno quedó pendiente. Antes de cada commit, `npm run build` limpio, `npm run lint` sin salida (código 0) y `npm run test` en verde: al final, **344 / 344** en 48 archivos. Nada de lo sin commitear de la otra sesión (`AGENTS.md`, una línea de `CLAUDE.md` y `visual-baseline.md`) se tocó: el `CLAUDE.md` del front se commiteó con un parche desde `HEAD` y `git apply --cached`.

| Hallazgo | Commit (front) | Qué se hizo |
| --- | --- | --- |
| Si fallaba el pedido nuevo de los roles, el editor se sembraba con la caché: no aparecían ni el error ni el sin permiso, y guardar mandaba la lista de permisos vieja | `ae7321f` | Se siembra solo con un pedido hecho después del montaje que terminó bien (`isSuccess` e `isFetchedAfterMount`). Tests desde el listado con un 500 (error, Reintentar y la siembra con lo nuevo) y con un 403 y la caché puesta. |
| Un 400 sobre la descripción caía arriba con el `detail` genérico, sin campo marcado; con dos, se perdía uno | `a7a583e` | Cada error va a su campo, todos a la vez; el de un campo que la pantalla no tiene va arriba con su propio texto. El nombre y la descripción tienen `maxLength` 64 y 256, los del backend. |
| Dos tests no cubrían lo que decían: el recorte del nombre y el sin permiso de `/roles/{id}` | `eeacc04` | El título y la miga se comparan exactos, con un caso de solo espacios, y el sin permiso prueba las dos rutas. Se vieron fallar con el código roto a propósito. |
| En inglés, el nombre accesible de “Pick all” y “Remove all” no empezaba con lo que se ve (WCAG 2.5.3) | `e48068b` | “Pick all permissions in {{area}}” y “Remove all permissions in {{area}}”, también en la tabla de textos de arriba. El test lo mira en los dos idiomas. |
| El foco caía en `<body>` al quitar un chip, al desmarcar o “Quitar todos” con “Elegidos”, y con “Ver todos los permisos” | `3438c72` | Pasa al siguiente de la misma lista, o al anterior si era el último; sin chips, al título del resumen; con la lista vacía, a “Ver todos los permisos”; y desde ahí, al buscador. |
| La banda adherida tapaba el foco con Shift+Tab (WCAG 2.4.11), e imprimir sacaba solo una pantalla | `f3db253` | `main` con `scroll-pt-14`, y `print:h-auto` y `print:overflow-visible`. Visto en Chrome con un arnés estático sobre el CSS construido (borrado): antes el foco quedaba detrás de la banda y `main` no se movía, ahora scrollea y queda a la vista; el PDF pasa de 1 hoja a 3, con todas las filas. La regla de `AppLayout` del `CLAUDE.md` del front lo cuenta, y el punto 3 de los pendientes también. |
| La columna se adhería 24 px debajo de la banda, y la nota de la Tarea 7 lo anotaba como “y no 20”: en el tablero se adhiere pegada a ella | `5e4a798` | `lg:top-14`: 24 px antes de scrollear, pegada a la banda al scrollear (en el arnés, 144 y 120). La nota quedó corregida. |
| La ayuda y el error de `FormField` miden 14 px y no 12,5, y el rótulo de solo lectura no va en gris | este commit | Solo documentado, en la nota de la Tarea 7 y en el punto 5 de los pendientes: es una pieza compartida de todos los formularios, y su biblioteca es de la otra sesión. |

**Lo que quedó:** las teclas de scroll con `main` como el que scrollea (punto 7 de los pendientes). No se arregló porque los dos caminos son una decisión que tiene que tomar el usuario: uno cambia el caparazón y la biblioteca, el otro a dónde va el foco al navegar.
