...
/// del email (sección 4 del spec de la Fase 4), salvo el del administrador inicial (<see cref="AccountCreationPolicy"/>).
/// </summary>
    ...
    : ICommandHandler<RequestLoginCodeCommand, RequestLoginCodeResponse>
{
    public async Task<Result<RequestLoginCodeResponse>> Handle(RequestLoginCodeCommand command, CancellationToken cancellationToken)
    {
        ...
        // El administrador inicial sí recibe el suyo, porque su cuenta se crea en su primer ingreso; la respuesta
        // es la misma, así que no dice cuál es su correo.
